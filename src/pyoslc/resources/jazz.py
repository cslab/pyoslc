...
class RootService(BaseResource):
    ...
    def to_rdf(self, graph):
        ...
        if self.publisher:
            publisher_url = url_for(
                "oslc.adapter_configuration_publisher", _external=True
            )
        ...
        tr.add(
            DCTERMS.type,
            URIRef(OSLC_CM.uri) if isinstance(OSLC_CM.uri, str) else OSLC_CM.uri,
        )
        tr.add(
            OSLC.domain,
        ...
        tr.add(
            OSLC.domain,
            URIRef(OSLC_AM.uri) if isinstance(OSLC_AM.uri, str) else OSLC_AM.uri,
        )
