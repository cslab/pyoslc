...
config_ns = Namespace(
    name="config", description="Configuration Management", path="/config"
)


register(
    "config-xml", Serializer, "pyoslc.serializers.configxml", "ConfigurationSerializer"
)
...
class ConfigurationComponent(OSLCResource):
    def get(self, component_id):
        endpoint_url = url_for(
            "{}.{}".format(request.blueprint, self.endpoint), component_id=component_id
        )
        base_url = "{}{}".format(request.url_root.rstrip("/"), endpoint_url)

        components_url = urlparse(base_url).geturl()

        provider = ConfigurationManagementSingleton.get_component(
            components_url, component_id
        )
        # provider.to_rdf(self.graph, config=True)

        response = make_response(
            render_template("pyoslc_oauth/components.html", dialog=provider.about)
        )
...
class ConfigurationPublisher(OSLCResource):
    def get(self):
        ...
        response = make_response(
            render_template("pyoslc_oauth/publisher.html", dialog=components_url)
        )
