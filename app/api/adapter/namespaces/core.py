...
register(
    "rootservices-xml",
    Serializer,
    "pyoslc.serializers.jazzxml",
    "JazzRootServiceSerializer",
)
...
@adapter_ns.route(
    "/provider/<service_provider_id>/resources/requirement/<requirement_id>/<preview_type>"
)
class ResourcePreviewSmallLarge(OSLCResource):
    def get(self, service_provider_id, requirement_id, preview_type):
        ...
        response = make_response(
            render_template(template, title="small", requirement=requirement)
        )
...
class ConfigurationCatalog(OSLCResource):
    def get(self):
        ...
        response = make_response(
            render_template(
                "pyoslc_oauth/configuration.html",
                about=catalog_url,
...
class ConfigurationPublisher(OSLCResource):
    def get(self):
        ...
        response = make_response(
            render_template("pyoslc_oauth/publisher.html", about=components_url)
        )
            ...
            result = [
                {
                    ...
                    "rdf:resource": url_for(
                        "oslc.adapter_configuration_stream", stream_id=1, _external=True
                    ),
                ...
                },
                {
                    ...
                    "rdf:resource": url_for(
                        "oslc.adapter_configuration_stream", stream_id=2, _external=True
                    ),
                    "rdf:type": "http://open-services.net/ns/config#Stream",
                },
            ]
...
class ConfigurationStream(OSLCResource):
    def get(self, stream_id):
        endpoint_url = url_for(
            "{}.{}".format(request.blueprint, self.endpoint), stream_id=stream_id
        )
        base_url = "{}{}".format(request.url_root.rstrip("/"), endpoint_url)
        ...
        service_provider_url = url_for(
            "oslc.adapter_service_provider",
            service_provider_id="Project-1",
            _external=True,
        )

        response = make_response(
            render_template(
                "pyoslc_oauth/stream.html",
                stream_url=stream_url,
                selection_url=url_for(
                    "oslc.adapter_configuration_selection", _external=True
                ),
                stream_id=stream_id,
