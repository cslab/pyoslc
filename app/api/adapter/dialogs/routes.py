...
dialog_bp = Blueprint(
    "dialog", __name__, template_folder="templates", static_folder="static"
)


@dialog_bp.route(
    "/provider/<service_provider_id>/resources/selector", methods=["GET", "POST"]
)
def index(service_provider_id):
    ...
    if resource_type:
        ...
        for r in requirements:
            if terms != "":
                if (
                    r.identifier.__contains__(terms)
                    or r.title.__contains__(terms)
                    or r.description.__contains__(terms)
                ):
                    results.append(
                        {
    ...
    return render_template(
        "dialogs/selector.html",
        selection_url=selection_url,
        selection_type_url=selection_type_url,
        form=form,
        list_req=list_req,
    )
...
@dialog_bp.route(
    "/provider/<service_provider_id>/resources/creator", methods=["GET", "POST"]
)
def create(service_provider_id):
    ...
    if form.validate_on_submit():
        ...
        results = [
            {
                "rdf:resource": creator_url.replace("creator", "requirement")
                + "/"
                + req.identifier,
...
def types(service_provider_id):
    results = list()
    results.append(
        {"oslc:label": "Specification", "rdf:resource": str("specification")}
    )
